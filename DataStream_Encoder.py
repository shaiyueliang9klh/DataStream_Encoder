...
class InfinityScope(ctk.CTkCanvas):
    ...
    def add_point(self, val):
        self.points.append(val); self.draw()
    def clear(self):
        self.points = []; self.max_val = 10.0; self.delete("all")
    def draw(self, event=None):
        self.delete("all")
        if not self.points: return
        w = self.winfo_width(); h = self.winfo_height(); n = len(self.points)
        if w < 10 or h < 10: return
        data_max = max(self.points) if self.points else 10
        target_max = max(data_max, 10) * 1.1 
        self.max_val += (target_max - self.max_val) * 0.1 if target_max > self.max_val else (target_max - self.max_val) * 0.05
        ...
        if len(coords) >= 4: self.create_line(coords, fill=COLOR_CHART_LINE, width=2, smooth=True)
...
class UltraEncoderApp(DnDWindow):
    def __init__(self):
        ...
        self.title("Ultra Encoder v36 - Fixed Architecture")
            ...
            if self.running and not self.stop_flag:
                    ...
                    with self.queue_lock:
                        for f in self.file_queue:
                            ...
                            if w and w.status_code == STATUS_WAIT:
                                self.after(0, lambda w=w: w.set_status("⏸️ 避让写入", COLOR_PAUSED) if w.winfo_exists() else None)
                                break
                ...
                try:
                    ...
                    if target_file and target_widget:
                        ...
                        if success and target_widget and target_widget.winfo_exists() and target_widget.status_code == STATUS_READ and self.active_moves == 0:
            ...
            with self.queue_lock:
                ...
                if slots_free > 0:
                    for f in self.file_queue:
                        ...
                        if self.task_widgets[f].status_code in [STATUS_WAIT, STATUS_READ, STATUS_READY]:
    ...
    def process(self, input_file):
        ...
        is_ready = (card.status_code == STATUS_READY)
        
        # [关键修复] 锁逻辑重写：使用标志位确保只释放一次
        lock_acquired = False
        if not is_ready: 
        ...
        try:
            if card.status_code in [STATUS_DONE, STATUS_MOVE]: 
            ...
            cmd = ["ffmpeg", "-y", "-i", input_file, "-c:v", "hevc_nvenc" if "H.265" in codec_sel else "h264_nvenc", "-pix_fmt", "yuv420p", "-rc", "vbr", "-cq", str(self.crf_var.get()), "-preset", "p6", "-spatial-aq", "1", "-c:a", "copy", temp_out]
