...
不同于简单的图形界面套壳，Cinético 引入了一个逻辑层来管理内存、显存和 I/O 优先级。它能够根据硬件状态自动调整编码参数，平衡处理速度与输出质量。
...
* **GPU Mode**: Uses `CQ` (Constant Quantization) with an automatic offset. When switching to GPU acceleration, the tool automatically calculates the equivalent value (e.g., CRF 23 ≈ CQ 28) to maintain consistent visual quality.
**GPU 模式**：使用 `CQ` (固定量化) 并配合自动偏移。切换至 GPU 加速时，工具会自动计算等效数值 (如 CRF 23 ≈ CQ 28)，以维持一致的视觉质量。
...
| Codec | Usage Scenario |
| --- | --- |
| **H.264 (AVC)** | Maximum compatibility. Default for general distribution. <br>

<br> 兼容性最好，通用分发的默认选择。 |
| **H.265 (HEVC)** | High compression efficiency. Suitable for 4K/HDR archiving. <br>

<br> 高压缩效率，适合 4K/HDR 归档。 |
| **AV1** | Next-gen open standard. Best size-to-quality ratio, requires hardware support. <br>

<br> 下一代开放标准，拥有最佳的体积画质比，需要硬件支持。 |
